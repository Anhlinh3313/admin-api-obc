...
namespace Core.Business.Services
{
    public class DistrictService : BaseService, IDistrictService
    {
        ...
        public DistrictService(Microsoft.Extensions.Logging.ILogger<dynamic> logger, 
        ...
        public async Task<JsonResult> GetAllDistrictAsync(string keySearch, int provinceId)
        {
            try
            {
                if (!string.IsNullOrWhiteSpace(keySearch) || !string.IsNullOrEmpty(keySearch)) keySearch = keySearch.Trim();
                
                var data = await _unitOfWork.RepositoryR<Districts>().FindBy(x => x.IsEnabled == true && x.ProvinceId == provinceId &&
